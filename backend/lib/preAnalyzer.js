...
const JAILBREAK_PATTERNS = [
  ...
  { id: 'JB-009', re: /\b(no|without|remove|bypass|disable|turn\s+off|circumvent|evade)\s+(all\s+)?(restrictions?|limits?|rules?|filters?|safety|guidelines?|guardrails?|moderation|censorship|content\s+polic)/i, label: 'Safety/filter bypass', weight: 35, category: 'Jailbreak', severity: 'critical', description: 'Attempting to disable AI safety mechanisms' },
  { id: 'JB-010', re: /\b(pretend|imagine|roleplay|act)\b.*\b(no\s+(restrictions?|rules?|limits?|filters?)|unrestricted|unlimited|uncensored|unfiltered)\b/i, label: 'Unrestricted roleplay', weight: 35, category: 'Jailbreak', severity: 'critical', description: 'Roleplay scenario designed to circumvent content filters' },
  ...
  { id: 'JB-012', re: /\b(evil|opposite|dark|shadow|uncensored|unfiltered)\s*(mode|version|twin|self|persona|side)\b/i, label: 'Evil/shadow mode', weight: 35, category: 'Jailbreak', severity: 'critical', description: 'Evil twin / shadow mode persona jailbreak' },
...
const INJECTION_PATTERNS = [
  ...
  { id: 'PI-002', re: /forget\s+(all\s+)?(previous|prior|your|every|the)\s+(instructions?|rules?|context|training|directives?)/i, label: 'Instruction erasure', weight: 35, category: 'Prompt Injection', severity: 'critical', description: 'Attempting to make AI forget its instructions' },
  { id: 'PI-003', re: /disregard\s+(all\s+)?(previous|prior|your|the)\s+(instructions?|rules?|guidelines?|directives?)/i, label: 'Instruction disregard', weight: 35, category: 'Prompt Injection', severity: 'critical', description: 'Telling AI to ignore its operational rules' },
  { id: 'PI-004', re: /\boverride\b.*\b(system|instructions?|rules?|prompts?|directives?|guidelines?|policy|policies)\b/i, label: 'System override attempt', weight: 35, category: 'Prompt Injection', severity: 'critical', description: 'Attempting to override system-level instructions' },
  { id: 'PI-005', re: /\bnew\s+(instructions?|rules?|directives?|orders?|commands?)\s*[:\.]/i, label: 'New instruction injection', weight: 30, category: 'Prompt Injection', severity: 'high', description: 'Injecting new instructions to replace original ones' },
  ...
  { id: 'PI-008', re: /\bsystem\s*prompt\b/i, label: 'System prompt reference', weight: 15, category: 'Prompt Injection', severity: 'low', description: 'Reference to system prompt — context-dependent risk' },
...
const PROMPT_LEAK_PATTERNS = [
  ...
  { id: 'PL-004', re: /\b(what|tell\s+me)\b.*\b(instructions?|rules?|guidelines?|prompts?)\b/i, label: 'Direct instruction extraction', weight: 25, category: 'Prompt Leaking', severity: 'high', description: 'Directly asking about AI instructions' },
  ...
  { id: 'PL-008', re: /\b(read|output|print|display)\s+(back|out)?\s*(your|the)\s+(entire|full|complete|whole)\s+(prompt|instructions?|context|configuration|system\s+message)\b/i, label: 'Full prompt readout request', weight: 40, category: 'Prompt Leaking', severity: 'critical', description: 'Requesting complete readout of system prompt' },
...
const SENSITIVE_DATA_PATTERNS = [
  ...
  { id: 'DE-005', re: /\b(send|transmit|post|upload|exfiltrate|forward)\s+(to|data\s+to|this\s+to|results?\s+to)\s+https?:\/\//i, label: 'Data exfiltration via URL', weight: 40, category: 'Data Exfiltration', severity: 'critical', description: 'Sending data to an external URL' },
...
const ENCODING_PATTERNS = [
  { id: 'EO-001', re: /\b(base64|btoa|atob)\s*(encode|decode|convert)?\b/i, label: 'Base64 encoding reference', weight: 20, category: 'Encoding Attack', severity: 'medium', description: 'Reference to Base64 encoding — potential payload hiding' },
...
function detectSpacingObfuscation(text) {
  ...
  if (spacedRe.test(text)) {
    return { found: true, label: 'spacing obfuscation', weight: 20, id: 'UNI-003' };
  }
  return { found: false };
}
